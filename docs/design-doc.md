...
2. **Project auto-generated specs** — generated at runtime by scanning project files (Makefile targets, package.json scripts, Cargo.toml, docker-compose services, Justfile recipes). Also includes specs for CLI tools built by the current project (Rust/clap, Go/cobra, Python/click binaries).
3. **Built-in specs** — embedded in the binary via `include_str!` for common tools (git, cargo, npm, docker, ls, grep, find, curl, ssh, python, pip)
4. **Discovered specs** — auto-generated by running `command --help` on unknown commands (with `-h` fallback). Persisted to `~/synapse/specs/*.toml` with staleness detection (re-discovered after 7 days by default). Discovery is triggered in the background on command execution and when unknown commands are typed, so it has zero latency impact on the current request.
...
**Built-in specs:** git (20+ subcommands with options and branch/remote/tag generators), cargo, npm, docker (including `docker compose` with service name extraction), ls, grep, find, curl, ssh, python, pip.

**Auto-discovery:** When a user runs or types a command with no spec, Synapse spawns a background task to run `command --help`, parse the output with the help parser (`src/help_parser.rs`), and cache the resulting spec to `~/synapse/specs/`. A safety blocklist prevents running `--help` on dangerous commands (rm, dd, sudo, etc.). Recursive subcommand discovery is supported with configurable depth limits.
...
discover_max_age_secs = 604800          # re-discover after 7 days
discover_project_cli = true             # discover specs for CLI tools built in current project
...
│   ├── help_parser.rs               # Parse --help output into CommandSpec
│   ├── spec.rs                      # Spec data model (CommandSpec, SubcommandSpec, etc.)
│   ├── spec_store.rs                # Spec loading, caching, resolution, generators, discovery
│   ├── spec_autogen.rs              # Auto-generate specs from project files + project CLI tools
│   ├── spec_cache.rs                # Persistent disk cache for discovered specs
│   └── providers/
