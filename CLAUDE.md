...
1. **Zsh Plugin** (`plugin/synapse.zsh`) — Thin shell layer providing NL translation mode (`? query` prefix), command execution reporting (warms spec caches), directory change reporting (pre-warms project spec cache), connection management, and daemon lifecycle. Uses `recursive-edit` with a `synapse-dropdown` keymap for NL result navigation.
...
3. **Background Tasks** — Demand-driven spec discovery on tab completion, compsys regeneration on discovery, dynamic completion serving.
...
- **Spec store** (`src/spec_store.rs`) — Caches project specs per-cwd (5min TTL), runs generators with timeout and caching. Manages demand-driven spec discovery (triggered on tab completion) and writes compsys completion files directly. Commands run in a sandboxed temp directory.
...
- **Auto-generation** (`src/spec_autogen.rs`) — Generates specs from project files. Dynamic tools (make, npm/yarn/pnpm/bun, docker-compose, just) use `GeneratorSpec` commands that run at completion time for always-current results. Static tools (Cargo.toml, Python) are parsed at spec generation time. When `auto_regenerate` is enabled (default), compsys files are written automatically on project cache miss.
- **Discovered specs** — Auto-generated by completion generators (for allowlisted tools) or `--help` regex parsing. Discovery is demand-driven: triggered on tab completion, not command execution. Written directly as compsys files to `~/.local/share/synapse/completions/`.
