...
- **Discovered specs** — Auto-generated by completion generators (for allowlisted tools) or `--help` regex parsing. Discovery is demand-driven: triggered on tab completion, not command execution. Written directly as compsys files to `~/.synapse/completions/`.
